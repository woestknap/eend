...
import Eend from './Eend'
import { Overlay } from '../src/components/Overlay'
// https://github.com/pmndrs/drei
import { useGLTF, Detailed, Environment, ScrollControls } from '@react-three/drei'
// https://github.com/pmndrs/react-postprocessing
// https://github.com/vanruesc/postprocessing
import { EffectComposer, DepthOfField, ToneMapping } from '@react-three/postprocessing'

function LittleBgDuck({ index, z, speed }) {
  const ref = useRef()
...
export default function LittleBgDucks({ speed = 1, count = 80, depth = 80, easing = (x) => Math.sqrt(1 - Math.pow(x - 1, 2)) }) {
  return (
    ...
    <Canvas id='canvas' gl={{ antialias: true }} camera={{ position: [0, 0, 10], fov: 20, near: 0.01, far: depth + 15 }}>
      ...
      {Array.from({ length: count }, (_, i) => <LittleBgDuck key={i} index={i} z={Math.round(easing(i / count) * depth)} speed={speed} /> /* prettier-ignore */)}
      ...
      <EffectComposer disableNormalPass multisampling={0}>
        <DepthOfField target={[0, 0, 0]} focalLength={1.0} bokehScale={14} height={700} />
